...
@property (nonatomic, weak) UIView *bottomLineView;//MyChange
@end
...
- (void)willMoveToSuperview:(UIView *)newSuperview {
    ...
    [self addScrollView];
    [self addMenuViewBottomLine];//MyChange
    [self addItems];
    [self makeStyle];
    [self addBadgeViews];
...
- (void)resetFrames {
    ...
    self.bottomLineView.frame = CGRectMake(0, frame.size.height+1, frame.size.width, 1);//MyChange
    [self resetFramesFromIndex:0];
}
...
//        item.normalColor = [self colorForState:WMMenuItemStateNormal atIndex:i];
        item.normalColor   = [UIColor grayColor];//MyChange
//        item.selectedColor = [self colorForState:WMMenuItemStateSelected atIndex:i];
        item.selectedColor   = [UIColor blackColor];//MyChange
...
//            item.font = [UIFont systemFontOfSize:item.selectedSize];
            item.font = [UIFont fontWithName:@"Helvetica-Bold" size:item.selectedSize];//MyChange
        }
